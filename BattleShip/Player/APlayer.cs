...
namespace Player
{
    ...
    class ShipTools
    {
        ...
        {
            Point tail = new Point(head.I + (Const.Const.loandong[direction] * (id))
                , head.J + (Const.Const.loancot[direction] * (id)));
            return tail;
        }
        public static void GetTail(Point head, ref Point tail, int direction, int id)
        {
            tail = new Point(head.I + (Const.Const.loandong[direction] * (id))
                , head.J + (Const.Const.loancot[direction] * (id)));
        }
    }
    public class AShip
    ...
    public class APlayer
    {
        ...
        public bool AbleToPutIn(int i, int j, int direction, int id)
        {
            ...
            if (direction == 1 || direction == 3)
            {
                ...
                {
                    if (sea[i][j + (k * Const.Const.loancot[direction])] != Const.Const.Unknow) return false;
                }
            }
            else
            {
                for (int k = 0; k < id + 1; k++)
                {
                    if (sea[i + (k * Const.Const.loandong[direction])][j] != Const.Const.Unknow) return false;
                }
            }
            return true;
        }
        ...
        void PutInValue(int i, int j, int direction, int length, int value)
        {
            if (direction == 1 || direction == 3)
            {
                ...
                {
                    sea[i][j + (k * Const.Const.loancot[direction])] = value;
                }
            }
            else
            {
                for (int k = 0; k < length; k++)
                {
                    sea[i + (k * Const.Const.loandong[direction])][j] = value;
                }
            }
        }
